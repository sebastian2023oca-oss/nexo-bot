...
const coinflipvs = {
    async ejecutar(sock, mensaje, args) {
        ...
        if (!mencionado || !args[args.length - 1]) {
            await sock.sendMessage(jid, { text: `❌ Uso correcto: *.coinflipvs @usuario <cantidad>*` }, { quoted: mensaje })
            return
        }
        ...
        if ((j2[0].monedas || 0) < cantidad) {
            await sock.sendMessage(jid, { text: `❌ @${mencionado.split('@')[0]} no tiene suficiente dinero para esta apuesta.`, mentions: [mencionado] }, { quoted: mensaje })
            return
        }
        ...
        await registrarJugadaCasino(ganador, 'coinflipvs', cantidad, 'gano', cantidad)
        await registrarJugadaCasino(perdedor, 'coinflipvs', cantidad, 'perdio', -cantidad)
