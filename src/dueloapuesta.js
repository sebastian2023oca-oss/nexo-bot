...
const dueloapuesta = {
    async ejecutar(sock, mensaje, args) {
        ...
        if (!mencionado || !args[args.length - 1]) {
            await sock.sendMessage(jid, { text: `❌ Uso correcto: *.dueloapuesta @usuario <cantidad>*` }, { quoted: mensaje })
            return
        }
        ...
        await registrarJugadaCasino(ganador, 'dueloapuesta', cantidad, 'gano', cantidad)
        await registrarJugadaCasino(perdedor, 'dueloapuesta', cantidad, 'perdio', -cantidad)

        await sock.sendMessage(jid, {
            text: `⚔️ *DUELO DE APUESTA*\n\n@${userJid.split('@')[0]} vs @${mencionado.split('@')[0]}\n💰 *Apuesta:* ${cantidad} monedas\n\n💥 @${ganador.split('@')[0]} usó *${ataque}* y ganó el duelo!\n\n🏆 *Ganador:* @${ganador.split('@')[0]} (+${cantidad})\n💀 *Perdedor:* @${perdedor.split('@')[0]} (-${cantidad})`,
            mentions: [userJid, mencionado]
        }, { quoted: mensaje })
    }
}

export default dueloapuesta