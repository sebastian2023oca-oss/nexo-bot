    ...
    async ejecutar(sock, mensaje, args) {
        ...
        if (!mencionado || !args[1]) {
            await sock.sendMessage(jid, {
                text: `❌ Uso correcto: *.transferir @usuario <cantidad>*\n\n📌 Ejemplo: *.transferir @juan 500*`
            }, { quoted: mensaje })
            return
        }
        ...
        if (receiver.length === 0) {
            await sock.sendMessage(jid, { text: `❌ El usuario mencionado no está registrado en el bot.` }, { quoted: mensaje })
            return
        }
